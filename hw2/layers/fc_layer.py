...
class FCLayer():
	def __init__(self, num_input, num_output, actFunction='relu', trainable=True):
		...
		self.grad_W = np.zeros((num_input, num_output)) # grad_W is used, need to be defined
		self.grad_b = np.zeros((1, num_output)) # grad_b is used, need to be defined
	...
	def backward(self, delta):
		...
		self.grad_W = np.matmul(self.input.T, delta)  # Calculate gradients for 1 <= l <= L
		self.grad_b = delta.sum(axis=0)  # Calculate gradients for 1 <= l <= L
		return np.matmul(delta, self.W.T)  # For hidden layers l, return delta^{l-1}
		############################################################################
